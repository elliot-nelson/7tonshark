...
> Technically, you can simplify the above to `${{ inputs.production || false }}`, and it will work fine. I have started avoiding this syntax, however, because it's too easy to replace that `false` with a `true` or a `"BLUE"` or a `5` or any other default, truthy value, and expect it to work; leaving a landmine like that for other developers who aren't as familiar with the nuances of GitHub Actions can result in disaster.
